...
import { z } from "zod";
import { moderateGroup } from "@/features/groups/services/groups-service";
...
export async function PATCH(
  ...
  try {
    ...
    const group = await moderateGroup({
      actorUserId: user.id,
      groupId: id,
    ...
    return NextResponse.json({ group }, { headers: getRateLimitHeaders(rateLimit) });
  } catch (error) {
